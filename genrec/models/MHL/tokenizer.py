...
class MHLTokenizer(AbstractTokenizer):
    """
    MHL Tokenizer - Same as RPG Tokenizer but with MASK token support
    ...
    def __init__(self, config: dict, dataset: AbstractDataset):
        ...
        super(MHLTokenizer, self).__init__(config, dataset)
        ...
        self.mask_token_id = self.eos_token + 1  # Add MASK token after EOS
    ...
    def vocab_size(self) -> int:
        """
        Returns the vocabulary size for the MHL tokenizer.
        Includes PAD, semantic tokens, EOS, and MASK.
        """
    ...
    def _encode_sent_emb(self, dataset: AbstractDataset, output_path: str):
        ...
        assert self.config['metadata'] == 'sentence', \
            'MHLTokenizer only supports sentence metadata.'
